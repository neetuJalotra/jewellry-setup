...
app.get('/api/customers', authenticateToken, async (req, res) => {
  try {
    const connection = await pool.getConnection();
    try {
      // Join Customers and Users to filter by role
      const [customers] = await connection.execute(
        `SELECT c.* FROM Customers c
                 JOIN Users u ON c.UserID = u.UserID
                 WHERE c.IsActive = TRUE AND u.Role = 'Customer'
...
// Notification Route: Notify customer for order pickup
app.post('/api/notify-pickup', authenticateToken, async (req, res) => {
  try {
    const { customerId, orderId, phone, message } = req.body;
    if (!customerId || !orderId || !message) {
      return res.status(400).json({
        success: false,
        message: 'customerId, orderId, and message are required'
      });
    }
    const connection = await pool.getConnection();
    try {
      await connection.execute(
        'INSERT INTO notifications (customer_id, order_id, message) VALUES (?, ?, ?)',
        [customerId, orderId, message]
      );
      // (Optional) Integrate SMS API here if needed
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'jalotra.2010@gmail.com',      // Replace with your email
    pass: 'ghvr gtqd giun wumv'    // Use an app password, not your main password
  }
});
