        ...
        <tr>
            ...
            <td>
                <div class="action-buttons">
                    ...
                    <button class="action-btn message-btn" onclick="showSendMessageModal('${customer.id}')">
...
// Notify customer for pickup (calls backend API)
function notifyCustomerPickup(order) {
  // Find customer info
  ...
  fetch('/api/notify-pickup', {
    ...
    headers: {
      ...
      // 'Authorization': 'Bearer ' + localStorage.getItem('token'), // Uncomment if using JWT auth
    },
    body: JSON.stringify({
    ...
    .then(data => {
      if (data.success) {
        showNotification('Customer notified for pickup!', 'success');
      } else {
        showNotification('Failed to notify customer.', 'error');
      }
    })
    .catch(() => showNotification('Failed to notify customer.', 'error'));
} 

// Show Send Message Modal
let selectedCustomerId = null;
function showSendMessageModal(customerId) {
  selectedCustomerId = customerId;
  const customer = customers.find(c => c.id === customerId);
  if (!customer) return;
  document.getElementById('sendMessageCustomerInfo').textContent = `To: ${customer.firstName} ${customer.lastName} (${customer.phone})`;
...
function sendMessageToCustomer() {
  ...
  fetch('/api/notify-pickup', {
    ...
    body: JSON.stringify({
      customerId: customer.id,
      orderId: '', // No orderId for general message
      phone: customer.phone,
      message: message
    })
  })
    ...
    .catch(() => showNotification('Failed to send message.', 'error'));
} 

function sendEmailToCustomer() {
  const customer = customers.find(c => c.CustomerID === selectedCustomerId || c.id === selectedCustomerId);
  if (!customer) return;
  const subject = document.getElementById('sendEmailSubject')?.value || 'Message from Jewellery Management';
  const message = document.getElementById('sendMessageText').value.trim();
  if (!message) {
    showNotification('Please enter a message.', 'error');
    return;
  ...
  fetch('/api/send-email', {
    ...
    .then(data => {
      if (data.success) {
        showNotification('Email sent to customer!', 'success');
        closeModal('sendMessageModal');
      } else {
        showNotification('Failed to send email: ' + (data.error || data.message), 'error');
        if (data.details) {
          alert('Error details: ' + data.details);
        }
      }
    })
    .catch((err) => showNotification('Failed to send email: ' + err, 'error'));
} 